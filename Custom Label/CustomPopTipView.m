...
#import "CustomPopTipView.h"

//! @cond   Private extension.
@interface CustomPopTipView ()
{
    CGSize					bubbleSize;
...
#pragma mark - Redeclarations

@property (unsafe_unretained, nonatomic, readwrite) id targetObject;
...
@implementation CustomPopTipView
...
- (void)drawRect:(CGRect)rect {
	...
	CGFloat alpha;
	int numComponents =
        CGColorGetNumberOfComponents(self.backgroundColor.CGColor);
    ...
    //Draw Border
    int numBorderComponents =
        CGColorGetNumberOfComponents(self.borderColor. CGColor);
    const CGFloat *borderComponents =
        CGColorGetComponents(self.borderColor.CGColor);
    CGFloat r, g, b, a;
...
- (void)presentPointingAtView:(UIView *)targetView inView:(UIView *)containerView animated:(BOOL)animated  withPointerPosition:(CGFloat)PointX {
	...
	CGFloat x_p = (PointX != 0)?PointX:p.x;
...
- (void)presentPointingAtBarButtonItem:(UIBarButtonItem *)barButtonItem animated:(BOOL)animated {
	...
	[self presentPointingAtView:targetView inView:containerView animated:animated withPointerPosition:0];
}
